...
3. **本地模型按"内存档位"分类，不锁死具体模型名**。新模型只要内存占用落进现有档位、写作质量验证通过，就可原地替换。当前两档：
   - **Tier A**：~8GB 总占用 → 现行 `Qwen3.5-9B Q4_K_S`
...
| 项 | 占用 |
|---|---|
| 模型权重 (Qwen3.5-9B IQ4_NL) | ~5.5 GB |
| 65536 ctx KV cache (Q8_0, hybrid arch) | ~1.0 GB |
| Gated DeltaNet recurrent state | ~0.5 GB |
| **小计** | **~7 GB** |

Qwen3.5-9B 是 Gated DeltaNet + Gated Attention 混合架构，32 层中**仅 8 层是真 Attention**，KV cache 不随上下文线性增长，原生支持 262K ctx。这是它能塞进 Tier A 的关键。
...
| binary | 状态 | 体积 |
|---|---|---|
| CUDA build (含 MTP, commit 1d7ab2b) | **v1 已打包** | ~45 MB |
...
- `llama-server.exe` (1.7 MB)
- `llama.dll`, `llama-common.dll`, `ggml.dll`, `ggml-base.dll`, `ggml-cpu.dll`, `mtmd.dll`
- `ggml-cuda.dll` (31 MB，CUDA backend)

**开发者环境准备**：从 `D:\AI相关\llama.cpp-new\build\bin\Release\`（或等效 CUDA + MTP build）拷贝上述 8 个文件到项目根 `local_llm/`。electron-builder 的 `extraResources` 配置（`electron/package.json`）会自动将 `local_llm/` 整目录复制进安装包，过滤掉 `models/**` 和 `*.md`。

**版本要求**：llama.cpp ≥ **2026-05-16 主干**（PR #22673 之后）。早期版本不识别 Qwen3.5 hybrid 架构和 Qwen3.6 MoE，整张表的本地分支都不可用。当前打包的 commit `1d7ab2b` 已含 MTP 支持。
...
- [unsloth/Qwen3.5-9B-GGUF](https://huggingface.co/unsloth/Qwen3.5-9B-GGUF) — Tier A 来源
